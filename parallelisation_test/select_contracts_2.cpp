...
*/
int f(string compiler, string flags, string path, string remains) 
{
...
*/
int f_wrapper(string compiler, string flags, string path, string remains)
{
  std::mutex m;
  std::condition_variable cv;
  int retValue;

  std::thread t([&m, &cv, &retValue, &compiler, &flags, &path, &remains]() 
      {
      retValue = f(compiler, flags, path, remains);
      cv.notify_one();
      });
...
void time_test(const vector<string> &sol_files, string test_string, 
	string compiler, string flags, string remains)
{
  //obtain num cpus
  ...
  f(compiler, flags, test_string, remains);
  ...
  if (cpus <= 1 || sol_files.size() == 1) {
    ...
    for (auto task : tasks) {
      vec_res.push_back(async(std::launch::async, f, compiler, flags, task, remains));
    }
...
int main(int argc, char** argv) 
{
  ...
  cout << test_string << '\n';
  time_test(sol_files, test_string, compiler, flags, remains);
  return 0; 
}
